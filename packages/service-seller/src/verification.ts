...
interface JwtCredentialPayload<T> {
  vc: T;
  iss: string;
  sub: string;
}

interface VerifiedCredential<T> {
  payload: JwtCredentialPayload<T>;
...
export const verifyPresentation = async (
  ...
  const parsedCreds = await Promise.all(
    credentials.map((vcJwt: string) => verifyCredential(vcJwt, log))
  );
...
const verifyJwt = async (
  ...
  return payload as JwtVpPayload;
};

const verifyCredential = async (vcJwt: string, log: LogFn): Promise<VerifiedCredential<any>> => {
  ...
  const payload = verification.payload as unknown as JwtCredentialPayload<any>;
  const vcType = Array.isArray((payload as any).vc?.type) ? (payload as any).vc.type[1] : "credential";
  log("vc.verified", "success", `Verified ${vcType} issued by ${issuerDid}`);
  return { payload, header };
};

const isEmailCredential = (
  record: VerifiedCredential<any>
): record is VerifiedCredential<EmailCredential> =>
  Array.isArray(record.payload.vc?.type) && record.payload.vc.type.includes(VC_TYPES.EMAIL);

const isAgeCredential = (
  record: VerifiedCredential<any>
): record is VerifiedCredential<AgeCredential> =>
  Array.isArray(record.payload.vc?.type) && record.payload.vc.type.includes(VC_TYPES.AGE);

const isDelegationCredential = (
  record: VerifiedCredential<any>
): record is VerifiedCredential<AgentDelegationCredential> =>
...
const decodeProtectedHeader = (jwt: string) => {
