...
export const createSellerService = (options = {}) => {
    ...
    const recordLog = (challengeId, step, status, detail, meta) => {
        const log = {
            id: nanoid(10),
            challengeId,
            step,
    ...
    router.get("/jobs", async (req, res) => {
        ...
        try {
            ...
            const outcome = await verifyPresentation(presentationHeader, challenge, (step, status, detail, meta) => recordLog(challenge.challengeId, step, status, detail, meta), forceUnder18, origin);
            ...
            const paymentResponseHeader = encodeBase64Url({
            ...
            const challengeId = error?.challengeId ??
                (typeof error === "object" && error !== null && "payload" in error
                    ? error.payload?.challengeId
                    : "unknown");
...
const mapVerificationError = (error) => {
    ...
    if (message.includes("expired") || error?.code === "ERR_JWT_EXPIRED") {
...
const decodePaymentEnvelope = (header) => {
    try {
        const json = Buffer.from(header, "base64url").toString("utf-8");
        return JSON.parse(json);
    }
    catch (error) {
        error.challengeId = "unknown";
        throw new Error("Invalid payment envelope");
    }
};
const encodeBase64Url = (payload) => Buffer.from(JSON.stringify(payload), "utf-8").toString("base64url");
const verifyPaymentSignature = async (envelope) => {
    const { payload, signature, kid } = envelope;
    if (!kid) {
        const err = new Error("Payment signature missing kid");
        err.challengeId = payload.challengeId;
        throw err;
    }
    const did = payload.payer;
    if (!did) {
        const err = new Error("Payment payload missing payer");
        err.challengeId = payload.challengeId;
        throw err;
    }
    const document = (await sharedDid.loadDidDocument(did));
    if (!document) {
        const err = new Error(`Unknown payer DID: ${did}`);
        err.challengeId = payload.challengeId;
        throw err;
    }
    const method = document.verificationMethod.find((method) => method.id === kid) ??
        document.verificationMethod[0];
    if (!method) {
        const err = new Error(`No verification method for DID ${did}`);
        err.challengeId = payload.challengeId;
        throw err;
    }
