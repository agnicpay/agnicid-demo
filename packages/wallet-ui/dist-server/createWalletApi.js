...
export const createWalletApi = (options = {}) => {
    ...
    router.post("/agent/run", asyncHandler(async (req, res) => {
        ...
        try {
            const result = await withBundleWorkspace(bundle, async () => {
                await ensureWalletStore();
                return executeX402Flow(jobs, { includeDelegation }, (event) => events.push(event));
            });
    ...
    router.post("/agent/inspect", asyncHandler(async (req, res) => {
...
const pathExists = (target) => storagePathExists(target);
const normalizeBundlePath = (value) => value
    .split(path.sep)
    .join("/")
    .replace(/^\/+/, "");
const KEY_EXCLUDE_SET = new Set(["keys/human.key.json", "keys/issuer.key.json", "keys/human.pub.json", "keys/issuer.pub.json"].map((entry) => entry.toLowerCase()));
const shouldIncludeInBundle = (relativePath) => {
...
const addDirectoryToZip = async (zip, absoluteDir, relativeDir, include) => {
...
const extractBundleTo = async (buffer, destination) => {
